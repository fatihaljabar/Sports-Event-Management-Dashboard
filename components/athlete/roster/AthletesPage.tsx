...
import { Search, ChevronDown, ShieldCheck, Plus, Eye, Trash2, Users } from "lucide-react";
import { AvatarCell } from "./AvatarCell";
import { StatusBadge } from "./StatusBadge";
import { AddAthleteModal } from "@/components/athlete/modals/AddAthleteModal";
import { ATHLETES, TEAMS } from "@/components/athlete/constants";
import type { AthleteRecord } from "@/components/athlete/types";
    ...
    <>
      ...
      <div className="p-6" style={{ fontFamily: "Inter, sans-serif" }}>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-5">
          ...
          >
            <span style={{ fontSize: "12px", color: "#6b7280" }}>Total Athletes:</span>
            <span style={{ fontSize: "13px", fontWeight: 700, color: "#2563eb" }}>224</span>
            <span style={{ color: "#d1d5db" }}>/</span>
            <span style={{ fontSize: "12px", color: "#9ca3af" }}>250</span>
          </div>
          ...
          >
            ...
            <span style={{ fontSize: "13px", fontWeight: 700, color: "#15803d" }}>218</span>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr style={{ backgroundColor: "#f8fafc", borderBottom: "1px solid #f3f4f6" }}>
                  ...
                  <th className="text-left px-4 py-3 uppercase tracking-wider" style={{ fontSize: "10px", fontWeight: 600, color: "#9ca3af" }}>Athlete</th>
              ...
              <tbody>
                {filtered.map((row: AthleteRecord, i: number) => (
                  <tr
                    key={row.id}
                    ...
                    onMouseEnter={(e) => { (e.currentTarget as HTMLTableRowElement).style.backgroundColor = "rgba(37,99,235,0.03)"; }}
                    onMouseLeave={(e) => { (e.currentTarget as HTMLTableRowElement).style.backgroundColor = i % 2 === 0 ? "#FFFFFF" : "#fafbfc"; }}
                  >
                    <td className="px-5 py-3.5">
                      <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "12px", color: "#475569", letterSpacing: "0.05em" }}>
                        {formatNik(row.nik)}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-2">
                        <div className="flex items-center justify-center rounded" style={{ width: "20px", height: "20px", backgroundColor: "#eff6ff" }}>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          style={{ fontSize: "12px", fontWeight: 500, borderColor: "#e5e7eb", color: "#4b5563" }}
                        >
                          <Eye size={13} /> View
                        </button>
                        <button
                          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border transition-colors"
                          style={{ fontSize: "12px", fontWeight: 500, borderColor: "#fee2e2", color: "#ef4444" }}
                        >
